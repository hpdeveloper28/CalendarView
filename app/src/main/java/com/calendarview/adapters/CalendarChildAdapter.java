...
import com.calendarview.customview.CustomTextView;
...
public class CalendarChildAdapter extends BaseAdapter {
     ...
     * @param extraData
     */
    public CalendarChildAdapter(Context context, int month, int year,
    ...
    private void resetCustomResources(CustomTextView customTextView) {
        customTextView.setBackgroundResource(defaultCellBackgroundRes);
        customTextView.setTextColor(defaultTextColorRes);
    }
     ...
     */
    protected void customizeTextView(int position, CustomTextView customTextView) {
        // Get the padding of cell so that it can be restored later
        int topPadding = customTextView.getPaddingTop();
        int leftPadding = customTextView.getPaddingLeft();
        int bottomPadding = customTextView.getPaddingBottom();
        int rightPadding = customTextView.getPaddingRight();

        // Get dateTime of this cell
        DateTime dateTime = this.datetimeList.get(position);

        customTextView.resetCustomStates();
        resetCustomResources(customTextView);

        if (dateTime.equals(getToday())) {
            customTextView.addCustomState(CustomTextView.STATE_TODAY);
        }

        // Set color of the dates in previous / next month
        if (dateTime.getMonth() != month) {
            customTextView.addCustomState(CustomTextView.STATE_PREV_NEXT_MONTH);
        }
        ...
        if ((minDateTime != null && dateTime.lt(minDateTime))
            ...
            customTextView.addCustomState(CustomTextView.STATE_DISABLED);
        }

        // Customize for selected dates
        if (selectedDates != null && selectedDatesMap.containsKey(dateTime)) {
            customTextView.addCustomState(CustomTextView.STATE_SELECTED);
        }

        customTextView.refreshDrawableState();

        // Set text
        customTextView.setText(String.valueOf(dateTime.getDay()));

        // Set custom color if required
        setCustomResources(dateTime, customTextView, customTextView);

        // Somehow after setBackgroundResource, the padding collapse.
        // This is to recover the padding
        customTextView.setPadding(leftPadding, topPadding, rightPadding,
                bottomPadding);
    }
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			customTextView = (CustomTextView) localInflater.inflate(squareDateCellResource, parent, false);
		} else {
			customTextView = (CustomTextView) convertView;
		}

		customizeTextView(position, customTextView);
